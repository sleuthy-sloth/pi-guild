...
|-------|-----------|------|
| Pi adapter | `pi/` | extension entry, `/studio` commands, `studio_*` tools |
...
seven default agent roles, and seeds the default policy set (safe actions
...
`ModelRouter` (see `core/orchestration/model-router.ts`), which maps role names
to a model and provider. A `/studio config` command is a planned follow-up.
...
| `/studio plane` / `github` | stubs — later milestones |
| `/studio start` | stub — background scheduler loop (later milestone) |

The same surface is available to agents as `studio_*` tools (`studio_list_tasks`,
`studio_create_task`, `studio_assign_task`, `studio_send_message`,
`studio_record_decision`, `studio_escalate_to_human`, …).

## Agent roles

Seven data-driven roles ship in `agents/` (each with `role.md`, `policy.json`,
`tools.json`, `prompt.md`):
...
| **Developer** | implement code, write tests, fix bugs | edit code, run bash, create PRs |
...
with **mocked runtimes — no real LLM calls**:

- `tests/unit/` — repository CRUD, task engine (decomposition, cycle rejection,
  readiness), scheduler, messaging, event bus, policies, memory, lifecycle.
- `tests/e2e/calculator.e2e.test.ts` — a full pipeline (org → project → roles →
  agents → epic → stories → tasks → dependencies → scheduler loop) driven by a
  fake `AgentRunner` that records every run and returns `{ ok: true }`.
